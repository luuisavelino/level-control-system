...
func (aw *advancedWorker) start() {
	...
	aw.action.mqtt.Subscribe(aw.data.GetPath(), 1, messageChannel)
	...
	go func() {
		for {
			...
			case message := <-messageChannel:
				...
				aw.action.mqtt.Publish(fmt.Sprintf("%s/action", aw.data.GetPath()), 1, false, actionControl)
...
func (aw *advancedWorker) stop() error {
	...
	err := aw.action.mqtt.Unsubscribe(aw.data.GetPath())
	if err != nil {
